...
const DEFAULT_MODEL_URL = "/models/MOSFET.glb";
...
function injectStyles() {
    if (typeof document === "undefined")
        return;
    if (document.getElementById("mosfet-viewer-style"))
        return;
...
function findContainer(selector) {
    ...
    const dataHook = document.querySelector("[data-mosfet-viewer]");
    if (dataHook)
        return dataHook;
    const fallback = document.getElementById("mosfet-viewer");
    return fallback;
}
function ensureMount(container, attr, className) {
    const existing = container.querySelector(attr);
    if (existing) {
...
function snapshotMesh(key, mesh, store) {
    if (!mesh)
        return;
...
function restoreMesh(key, mesh, store) {
    if (!mesh)
        return;
    ...
    materials.forEach((mat, idx) => {
        ...
        if (!snap)
            return;
        const anyMat = mat;
        if (anyMat.color && snap.color !== undefined)
            anyMat.color.setHex(snap.color);
        if (anyMat.emissive && snap.emissive !== undefined)
            anyMat.emissive.setHex(snap.emissive);
        if (snap.emissiveIntensity !== undefined)
            anyMat.emissiveIntensity = snap.emissiveIntensity;
        if (snap.opacity !== undefined)
            anyMat.opacity = snap.opacity;
        if (snap.transparent !== undefined)
            anyMat.transparent = snap.transparent;
        if (snap.depthWrite !== undefined)
            anyMat.depthWrite = snap.depthWrite;
        if (snap.visible !== undefined)
            mat.visible = snap.visible;
    });
}
function setGlow(mesh, hexColor, intensity) {
    if (!mesh)
        return;
...
function setTransparency(mesh, opacity) {
    ...
    getMaterials(mesh).forEach((mat) => {
        ...
        if (anyMat.depthWrite !== undefined)
            anyMat.depthWrite = false;
    });
}
function attachLabel(target, text, anchor) {
    if (!target)
        return null;
...
export function initMosfetViewer(options = {}) {
    ...
    if (container[AUTO_ATTACH_KEY]) {
        return container[AUTO_ATTACH_KEY];
    }
    injectStyles();
    ...
    function applyGateState(on) {
        ...
        if (currentArrow) {
            ...
            if (on)
                setGlow(currentArrow, 0x32d5ff, 1.6);
        }
        if (leakArrow) {
            const showLeak = !on && state.leakCandidate;
            leakArrow.visible = showLeak;
            if (showLeak)
                setGlow(leakArrow, 0xffb347, 0.45);
        }
    }
    function updateChannelLength(t) {
        state.channelT = t;
        ...
        const leakArrow = getMesh(nodes.LeakArrow);
        if (source)
            source.position.x = -halfGap;
        if (drain)
            drain.position.x = halfGap;
        if (channelMesh)
            channelMesh.scale.x = scaleX;
        if (currentArrow)
            currentArrow.scale.x = scaleX;
        if (leakArrow)
            leakArrow.scale.x = scaleX;
    ...
    loader.load(options.modelUrl ?? DEFAULT_MODEL_URL, (gltf) => {
        const root = gltf.scene.getObjectByName("PlanarMOSFET_Root") ?? gltf.scene;
        scene.add(root);
        ...
        if (gateOxide)
            setTransparency(gateOxide, 0.32);
        if (ild)
            setTransparency(ild, 0.32);
    ...
    const dispose = () => {
        ...
        container[AUTO_ATTACH_KEY] = undefined;
    };
    container[AUTO_ATTACH_KEY] = dispose;
    return dispose;
}
if (typeof window !== "undefined" && typeof document !== "undefined") {
    const autoStart = () => {
